...
public class SubWarehouse {
	...
	public int optimizeHelper(int NumOfItems, int Weight) {

		if (NumOfItems == 0)
			return 0;
		if (Weight == 0)
			return 0;
		if (itemWeights.get(NumOfItems - 1) > Weight) {
			return optimizeHelper(NumOfItems - 1, Weight);
		} else {
			return max(
					itemProfits.get(NumOfItems - 1)
							+ optimizeHelper(NumOfItems - 1, Weight - itemWeights.get(NumOfItems - 1)),
					optimizeHelper(NumOfItems - 1, Weight));
		}
	}

	public void optimizeSelection() {

		// Implementing a Prolog-like approach, we can check recursively for the maximum profit,
		// by recursively calling a resulting maxProfit and finally checking if the
		// value is higher with or without the item at said index

		maxChosenProfit = optimizeHelper(itemWeights.size(), maxCapacity);

		String email = "amr.nader@student.guc.edu.eg";
